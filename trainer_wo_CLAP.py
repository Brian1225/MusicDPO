...
from transformers import AutoProcessor, AutoModelForTextToWaveform, EncodecModel, BitsAndBytesConfig, default_data_collator, AutoFeatureExtractor, ClapModel
...
def get_bnb_config() -> BitsAndBytesConfig:
    ...
    #     llm_int8_skip_modules=["BatchNorm", "LayerNorm"],  # Skip specific layers
    # )
    return bnb_config
...
#     similarity = F.cosine_similarity(audio_embeddings.unsqueeze(1), text_embeddings.unsqueeze(0), dim=-1)
...
class DPOTrainer():
    def __init__(self, args, dataset):
        ...
        # self.clap_model = ClapModel.from_pretrained("laion/clap-htsat-unfused", quantization_config=self.bnb_config)
        # self.clap_ckpt = torch.load(self.args.clap_ckpt)
        # self.clap_model.load_state_dict(self.clap_ckpt)
    ...
    def load_model(self):
        ...
        no_decay = ["BatchNorm", "LayerNorm.weight"]
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for time_step, batch in enumerate(pbar):
                ...
                # decode the audio tokens to audio values
                # generated_tokens = torch.argmax(model_audio.logits, dim=-1)
                # generated_tokens = generated_tokens.view(1, text.shape[0], self.model.decoder.num_codebooks, -1)
                # output_audio = self.encoder.decode(generated_tokens, [None])[0]

                # get audio embeddings
                # output_audio_embed = self.feature_extractor(output_audio.squeeze(1).detach().cpu().numpy().astype("float32"), sampling_rate=48000, return_tensors="pt")['input_features'].squeeze(0)
                # output_clap = self.clap_model(input_ids=text, attention_mask=attn_mask, input_features=output_audio_embed.half().to(self.DEVICE))
                # audio_embed = output_clap['audio_embeds']
                # text_embed = output_clap['text_embeds']

                # contrastive loss from input text and output audio embeddings
                # loss_clap = contrastive_loss(audio_embed, text_embed)
                # loss += loss_clap
                loss += decode_loss
...
def main():
    ...
    parser.add_argument("--processor_path", type=str, default="facebook/musicgen-small")
    parser.add_argument("--model_path", type=str, default="facebook/musicgen-small")
    ...
    parser.add_argument("--wandb_run_name", type=str, default="musicgen-dpo-small")
    parser.add_argument("--clap_ckpt", type=str, default='./clap_model_epoch_29.pth')
